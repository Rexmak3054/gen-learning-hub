...
async def chat_loop(session):
    ...
    while True:
        ...
        try:
            ...
            # Display the response - show all messages to see the conversation
            messages = agents_client.messages.list(thread_id=thread.id, order=ListSortOrder.ASCENDING)
            # Show only the latest messages (skip ones we've already seen)
            for message in messages:
                if message.text_messages:
                    last_msg = message.text_messages[-1]
                    # Skip user messages we just sent
                    if message.role != MessageRole.USER or last_msg.text.value != user_input:
                        print(f"\n{message.role}:\n{last_msg.text.value}\n")
