...
class SimpleResearchAgent:
    ...
    def _build_graph(self, StateGraph, MessagesState, ToolNode, tools_condition, START, HumanMessage, SystemMessage):
        ...
        When a user asks for courses or learning resources in natural language, you should:
        1. Understand what they want to learn (e.g., "I want to learn Python" means they want Python programming courses)
        2. Use internal_vector_search_courses to find relevant courses in our database
        3. If not enough courses found, use external_search_courses to find more options
        4. If they mention a specific course UUID, use get_course_details_by_uuid for complete details
        
        Be smart about interpreting their requests:
        - "I want to learn web development" → search for web development courses
        - "Find me Python courses" → search for Python programming courses  
        - "Help me with machine learning" → search for machine learning courses
        - "I'm a beginner at coding" → search for beginner programming courses
        
        After calling the tools, respond with ONLY the course data returned by the tools in JSON format. Do not add commentary or explanations.
        """
    ...
    async def process_chat_message_stream(self, session_id: str, user_message: str, session_manager) -> AsyncGenerator[str, None]:
        """Process a user message and stream step-by-step agent responses in real-time"""
        logger.info(f"🌊 Streaming chat message processing for session {session_id}: '{user_message}'")
        
        try:
            # Step 1: Add user message to session first
            ...
            yield self._format_stream_message("message_added", thinking_msg)
            await asyncio.sleep(0.5)  # Simulate thinking time
            
            # Step 3: Agent searching message
            searching_msg = session_manager.add_message(
                session_id,
                "agent_searching", 
                "Searching our course database for relevant matches...",
            ...
            yield self._format_stream_message("message_added", searching_msg)
            await asyncio.sleep(1.0)  # Simulate search time
            
            # Step 4: Perform the actual search
            search_result = await self.search_courses_from_chat(user_message)
            
            if search_result["success"] and search_result["courses"]:
                # Step 5: Found courses message
                found_msg = session_manager.add_message(
                    session_id,
                    "agent_searching",
                    f"Great! I found {len(search_result['courses'])} relevant courses. Let me also check external sources for more options...",
                ...
                yield self._format_stream_message("message_added", found_msg)
                await asyncio.sleep(1.0)  # Simulate external search time
                
                # Step 6: Courses result message
                courses_msg = session_manager.add_message(
                    session_id,
                    "agent_courses",
                    f"Perfect! Here are the best courses I found for you:",
                ...
                yield self._format_stream_message("message_added", courses_msg)
                ...
                yield self._format_stream_message("message_added", followup_msg)
                ...
                yield self._format_stream_message("message_added", no_results_msg)
            
            # Final stream completion message
            yield self._format_stream_message("stream_complete", {
                "session_id": session_id,
                "total_messages_added": len(search_result.get("courses", [])) + 4,
                "status": "success"
            })
...
async def serve_demo():
    """Serve the streaming chat demo HTML"""
    try:
        # Read the HTML file
